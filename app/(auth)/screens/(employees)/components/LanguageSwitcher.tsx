...
export const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({
  ...
  const languages = [
    {
      language: "EN",
      flag: require("../../../../../assets/images/flag-france.png"),
    },
    {
      language: "FR",
      flag: require("../../../../../assets/images/flag-france.png"),
  ...
  return (
    <View
      style={tw`flex text-xs self-stretch text-center whitespace-nowrap max-w-[66px] w-66 text-zinc-600`}
    >
      <View
        style={tw`flex flex-rowgap-2.5 px-1.5 py-1.5 self-stretch bg-white shadow-[0px_2px_4px_rgba(0,0,0,0.25)]`}
      >
        <TouchableOpacity onPress={() => setIsOpen((prev) => !prev)}>
          <View style={tw`flex flex-row items-center self-stretch gap-2`}>
            <View style={tw`grow`}>
              <Text
                style={tw`grow text-xs text-center whitespace-nowrap text-zinc-600`}
              >
                {defaultLanguage}
              </Text>
            </View>

            <LanguageIcon imageUrl={icon} alt={icon.alt} />
            <Image
