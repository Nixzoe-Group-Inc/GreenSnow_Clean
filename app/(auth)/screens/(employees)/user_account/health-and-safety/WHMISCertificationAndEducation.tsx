...
const WHMISCertificationandEducation = () => {
  ...
  return (
    <SafeAreaView style={tw`flex-1 px-4 py-10 bg-gray-100`}>
      ...
      <Text style={tw`self-start text-2xl font-bold text-stone-900`}>
        WHMIS Certification and Education
      </Text>
      ...
      <ScrollView style={tw`flex w-full`} showsVerticalScrollIndicator={false}>
        ...
        <View style={tw`flex`}>
          ...
          <View style={tw`pt-4`}>
            {[
              {
                label: "Health & Safety Introduction",
                icon: images.health,
              ...
              },
              {
                ...
                icon: images.health,
                route:
                  "/(employees)/user_account/health-and-safety/WHMISCertificationAndEducationAgreement",
              },
              {
                label: "Violence and Sexual Harassment Policy",
...
const DisplayMenuInfo: React.FC<DisplayPersonalInfoProps> = ({
  item,
  isVerified,
}) => {
  return (
      ...
      <View style={tw`flex flex-row gap-3 items-start`}>
        <View style={tw`flex items-center justify-center mt-1`}>
          <View style={tw`flex w-[18px] h-[18px] items-center justify-center`}>
            <Image source={images.document} /> // all items have same icon
            {!isVerified && (
              <Image
...
export default WHMISCertificationandEducation;
